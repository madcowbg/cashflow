...
describe("mean_reversion", () => {
  it("should have specific value", () => {
    const sample = take(10)(
      random_mean_reverting(2, 3, 0.2, 0.2)(standard_white_noise.pick(124123))
    );
  ...
  it("should return the same process for the same seed", () => {
    expect(
      take(100)(
        random_mean_reverting(2, 3, 0.2, 0.2)(standard_white_noise.pick(123))
      )
    ).to.deep.eq(
      take(100)(
        random_mean_reverting(2, 3, 0.2, 0.2)(standard_white_noise.pick(123))
  ...
  it("should return processes with uncorrelated shocks for (seed) vs (seed + 1)", () => {
    ...
    const b = diff(
      take(1000)(
        random_mean_reverting(2, 3, 0.2, 0.2)(standard_white_noise.pick(124))
      )
    );
  ...
  it("should take the same random values if called several times (stabilized)", () => {
    const process = random_mean_reverting(
      2,
      3,
      0.2,
      0.2
    )(standard_white_noise.pick(123));
